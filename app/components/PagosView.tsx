...
export default function PagosView({ refreshTrigger = 0 }: Props) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [pR, padR, cR] = await Promise.all([
      ...
      if(!pR.ok||!padR.ok||!cR.ok) throw new Error();
      setPagos(await pR.json());
      setPadres(await padR.json());
  ...
  const meses = Array.from(new Set(pagos.map(p => p.fecha.slice(0,7)))).sort().reverse();
  const filtered = pagos.filter(p => {
    if(filterForma!=='todos' && p.forma!==filterForma) return false;
    if(filterPadre!=='todos' && p.padreId!==parseInt(filterPadre)) return false;
    if(filterMes!=='todos' && !p.fecha.startsWith(filterMes)) return false;
    if(search) { const pad=padreMap[p.padreId]; const q=search.toLowerCase(); if(!pad?.nombre.toLowerCase().includes(q) && !pad?.cedula.includes(q) && !p.numRecibo.toLowerCase().includes(q)) return false; }
    return true;
  }).sort((a,b) => b.fecha.localeCompare(a.fecha));

  const totalMonto = filtered.reduce((s,p) => s+p.monto, 0);
  const totalFacturas = filtered.reduce((s,p) => s+p.facturasCubiertas.reduce((a,f) => a+f.abono, 0), 0);
  ...
  const printTable = () => {
    ...
    <tr class="tot"><td colspan="4">TOTAL — ${filtered.length} pago(s)</td><td>${formatMoney(totalMonto)}</td><td></td><td></td></tr>
  ...
  const toExcel = () => {
    ...
    ws.push([], [`TOTAL: ${filtered.length} pagos`, `Monto total: RD$${totalMonto}`]);
    ...
    <div>
      <div style={{display:'flex',gap:'8px',marginBottom:'12px',flexWrap:'wrap',alignItems:'center'}}>
        ...
        <select className="inp" value={filterMes} onChange={e=>setFilterMes(e.target.value)} style={{width:'auto'}}>
          <option value="todos">Todos los meses</option>{meses.map(m=><option key={m} value={m}>{m}</option>)}
        </select>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))',gap:'10px',marginBottom:'14px'}}>
        ...
        <div className="card" style={{padding:'10px 14px'}}><div style={{fontSize:'9px',fontWeight:700,color:'var(--hc-gray)',textTransform:'uppercase',marginBottom:'3px'}}>Pagos Registrados</div><div style={{fontFamily:"'Playfair Display',serif",fontSize:'16px',fontWeight:700}}>{filtered.length}</div></div>
      </div>
      ...
      <div className="card" style={{overflowX:'auto'}}>
        <div className="tbl-wrap">
          <table>
            ...
            <tbody>
              {filtered.length===0 && <tr><td colSpan={9} style={{textAlign:'center',color:'var(--hc-gray)',padding:'20px'}}>Sin pagos con los filtros seleccionados</td></tr>}
              {filtered.map(p => {
                const pad = padreMap[p.padreId];
                ...
                return (
                  <>
                    <tr key={p.id} style={{cursor:'pointer'}} onClick={() => toggleExpand(p.id)}>
                      <td style={{width:'28px',textAlign:'center',color:'var(--hc-gray)',fontSize:'12px'}} title="Expandir detalle">{isExpanded ? '▼' : '▶'}</td>
                            ...
                            <table style={{width:'100%',borderCollapse:'collapse',fontSize:'12px'}}>
                              ...
                              <tbody>
                                {(p.facturasCubiertas||[]).map(fc => {
                                  ...
                                  const isFullPayment = abono >= monto - (pagado - abono);
